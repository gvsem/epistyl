...
describe("runtime/replica transactions", () => {
    ...
    it("issueTransaction supports custom txId", () => {
        const replica = createReplicaState("A");
        ...
        expect(issued.transaction.txId).toBe("custom-tx-id");
        expect(issued.transaction.operations[0]?.txId).toBe("custom-tx-id");
    });
    ...
    it("supports transaction labels and timestamps without affecting materialized state", () => {
        ...
        const history = replica.objects[OBJECT_ID];
        expect(history?.operations.every((op) => op.timestamp === "2026-03-13T15:00:00Z")).toBe(true);
